...
This produces `package-visualizer-vscode-0.1.0.vsix`.

### 2. Install in VSCode

```bash
code --install-extension package-visualizer-vscode-0.1.0.vsix
```
...
node esbuild.mjs && node esbuild.webview.mjs
npx vsce package --no-dependencies
code --install-extension package-visualizer-vscode-<version>.vsix
```
